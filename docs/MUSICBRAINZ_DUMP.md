...
MusicBrainz 官方 Dump 适合作为开放基础目录，但完整 PostgreSQL 关系、编辑历史、标签和派生内容不适合直接装入当前单节点 SQLite。MVP 只导入 artist、release、recording、track 及其有序关系，不导入封面、歌词、编辑记录、用户评分或外部受限 payload。
...
```powershell
go run ./cmd/catalog-import -data-dir ./data -input ./musicbrainz-catalog.jsonl
```
...
导入前在预发布环境记录数据库大小、各实体计数、导入耗时和典型查询延迟。若目标子集超出单机磁盘/维护窗口或查询退化，缩小地区/时间/使用驱动子集；全量持续同步应推迟到 PostgreSQL 与独立 Worker（C5104），而不是增加 SQLite 并发写入。
